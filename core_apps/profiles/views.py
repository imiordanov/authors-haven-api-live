...
## The following lines are to illustrate function-based querying
# @api_view(["GET"])
...
def get_my_followers(request, username):
    try:
        ...
        raise NotFound("User with given username does not exist")
...
class FollowUnfollowAPIView(generics.GenericAPIView):
    ...
    def post(self, request, username):
        ...
        message = f"Hi there {specific_user.username}! The user {current_user_profile.user.username} is now following you!"
        ...
        return Response({
            "status_code": status.HTTP_200_OK,
            "detail": f"Now you follow {specific_user.username}"
        })
